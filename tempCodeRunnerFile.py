...
icons = {
    "hamburger": pygame.transform.scale(pygame.image.load(os.path.join("assets/img", "hamburger_button.png")),
                                        (40, 30)),
...
menu_background = pygame.transform.scale(pygame.image.load(os.path.join("assets/img", "menu2.png")),
    ...
    for row in range(8):
        for col in range(8):
            ...
            pygame.draw.rect(screen, color,
                             (offset[0] + col * tile_size, offset[1] + row * tile_size, tile_size, tile_size))
    if selected_square:
        pygame.draw.rect(screen, highlight, (
        offset[0] + selected_square[1] * tile_size, offset[1] + selected_square[0] * tile_size, tile_size, tile_size),
                         3)
    ...
    if selected_square is not None:
        ...
        for move in board.legal_moves:
            if move.from_square == from_sq:
                ...
                pygame.draw.rect(screen, color,
                                 (offset[0] + col * TILE_SIZE, offset[1] + row * TILE_SIZE, TILE_SIZE, TILE_SIZE), 3)
...
def draw_top_bar(screen):
    ...
    screen.blit(turn_surface,
                (WINDOW_WIDTH - turn_surface.get_width() - 10, (TOP_MARGIN - turn_surface.get_height()) // 2))
    return rects
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if one_player_rect.collidepoint(pos):
                    ...
                    engine = ai.TimeLimitedEngine(time_limit=15, max_depth=10)
                    board.reset()
                    selected_square = None
                    undone_moves = []
